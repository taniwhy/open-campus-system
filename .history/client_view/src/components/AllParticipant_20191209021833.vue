    ...
    async loadList() {
      ...
      try {
        ...
        for (var subjectKey in this.subjectList) {
          ...
          valueBuf.name = key;
          //順番に参加者履歴から取り出し
          for (var resKey in this.res.data) {
            //もし学科名と参加学科が一致したら
            if (this.res.data[resKey].join_subject == key) {
              valueBuf.participant_num += 1;
              if (this.res.data[resKey].school_year == "1学年") {
                valueBuf.first += 1;
              } else if (this.res.data[resKey].school_year == "2学年") {
                valueBuf.second += 1;
              } else if (this.res.data[resKey].school_year == "3学年") {
                valueBuf.third += 1;
              } else if (this.res.data[resKey].school_year == "4学年") {
                valueBuf.forth += 1;
              } else {
                valueBuf.other += 1;
              }
