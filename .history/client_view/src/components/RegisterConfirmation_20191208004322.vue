      ...
      <div class="text-center">
        <v-overlay :value="overlay" color="grey darken-4">
          ...
          <div v-if="succeeded_register">
            <h2>登録完了しました!</h2>
            <v-btn x-large flat to="/complete" class="ma-2" app color="cyan" dark width="120px">OK</v-btn>
          </div>
...
export default {
  ...
  methods: {
    ...
    entryRegister(response) {
      this.data.participantHistoryForm.id = response.data.id
      this.data.participantHistoryForm.joinDay = moment().format('l');
      this.data.participantHistoryForm.joinSubject = this.data.joinSubject;
      axios
        .post("http://127.0.0.1:8000/api/participant_history/", this.data.participantHistoryForm, {
          auth: { username: settings["name"], password: settings["pass"] }
        })
