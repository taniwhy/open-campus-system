          ...
          <v-card ref="form" width="520px">
            <v-card-text>
              ...
              () => !!data.form.first_name_reading && data.form.first_name_reading.length <= 15 || '※15文字以内で入力してください',
                ...
                <v-row style="height: 70px;">
                  <v-col style="height: 70px;">
                    <v-select :items="data.years_list" label="年" dense solo></v-select>
                  </v-col>
                  <v-col style="height: 70px;">
                    <v-select :items="data.years_list" label="月" dense solo></v-select>
                  </v-col>
                  <v-col style="height: 70px;">
                    <v-select :items="data.days_list" label="日" dense solo></v-select>
                  </v-col>
                </v-row>
...
export default {
  ...
  methods: {
    create_birth: function() {
      for (var y = 2019; i > 1900; i--) {
        this.data.years_list.push(i);
      }
      for (var m = 1; i < 12; i++) {
        this.data.months_list.push(i);
      }
      for (var d = 1; i < 31; i++) {
        this.data.days_list.push(i);
      }
    },
