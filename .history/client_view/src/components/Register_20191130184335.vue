          ...
          <v-card ref="form" width="520px">
            <v-card-text>
              ...
              () => 1900 <= form.birth_year && form.birth_year <= 2019 || '※入力が無効です',
                  ...
                  <v-col style="height: 70px;">
                    <v-text-field
                      ...
                      :rules="[
              () => !!form.birth_year || '※入力されていません',
              () => 1 <= form.birth_year && form.birth_year <= 12 || '※入力が無効です',
              ]"
                      :error-messages="errorMessages"
                      placeholder="月"
                      height="15px"
              ...
              () => !!form.school_name && form.school_name.length <= 32 || '※32文字以内で入力してください',
                  ...
                  <v-select :items="school_year_list" v-model="form.school_year" filled label="選択してください"></v-select>
                  <v-row style="height: 25px;">
                ...
                <v-select :items="subject_list" v-model="form." filled label="選択してください"></v-select>
  ...
  methods: {
    subject_push: function(response) {
      for (var key in response) {
        this.subject_list.push(response[key].subject_name);
      }
    },
    register: function() {
