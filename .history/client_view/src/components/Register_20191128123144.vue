        ...
        <v-card ref="form">
          <v-card-text>
            ...
            <v-radio-group v-model="row" row>
              <v-radio label="学生" value="yes1" @change="student_check = !student_check"></v-radio>
              <v-radio label="その他" value="no1" @change="student_check = !student_check"></v-radio>
            </v-radio-group>
            ...
            <div v-if="!student_check">
              高校卒業済み
              <v-radio-group v-model="row" row>
              <v-radio label="はい" value="yes2" @change="student_check = !student_check"></v-radio>
              <v-radio label="いいえ" value="no2" @change="student_check = !student_check"></v-radio>
            </v-radio-group>高等学校卒業程度認定試験取得済み
              <v-radio-group v-model="row" row>
              <v-radio label="はい" value="radio-1" @change="student_check = !student_check"></v-radio>
              <v-radio label="いいえ" value="radio-2" @change="student_check = !student_check"></v-radio>
            </v-radio-group>
            </div>参加希望学科
            <v-select :items="gakka_items" filled label="選択してください"></v-select>
            <v-card-actions>
              <v-col align="center" justify="start">
                <v-btn class="white--text" x-large color="light-blue lighten-3" width="120px">登録</v-btn>
              </v-col>
            </v-card-actions>
          </v-card-text>
        </v-card>
